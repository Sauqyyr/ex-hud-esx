...
RegisterNetEvent('hud:client:ToggleAirHud', function(data)
    showAltitude = not showAltitude
end)

AddEventHandler('esx_status:onTick', function(data) -- Triggered in qb-core
...
local function getFuelLevel(vehicle)
    ...
    if (updateTick - lastFuelUpdate) > 2000 then
        ...
        lastFuelCheck = math.floor(exports['nabstore-bensin']:GetFuel(vehicle))
    end
    ...
    while true do
        if LocalPlayer.state.isLoggedIn then
            ...
            if IsPedInAnyVehicle(ped, false) and not IsThisModelABicycle(GetEntityModel(GetVehiclePedIsIn(ped, false))) then
                if exports['nabstore-bensin']:GetFuel(GetVehiclePedIsIn(ped, false)) <= 20 then -- At 20% Fuel Left
                ...
                if IsPedInAnyVehicle(ped, false) then
                    ...
                    if config.VehClassStress[tostring(vehClass)] and not config.WhitelistedVehicles[vehHash] then
                        ...
                        if speed >= stressSpeed then
                            ...
                            TriggerEvent('esx_status:add', 'stress', math.random(500,1000))
                        end
                    end
                end
            end
